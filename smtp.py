...
class SMTP_(threading.Thread):
    def __init__(self, threadID, name, proxy_host, proxy_port, proxy_user, proxy_pass, user, passwd, FIRSTFROMNAME, LASTFROMNAME, target):
    ...
    def run(self):
        try:
            ...
            for index, item in self.target.iterrows():
                ...
                time.sleep(var.delay_between_emails)
            server.quit()
            server.close()
        except Exception as e:
...
def main(group):
    ...
    while var.send_campaign_email_count < target_len and group['flag'].sum() < group_len:
        ...
        for index, item in group.loc[group['flag'] == 0].iterrows():
            ...
            print(index, name, proxy_host, proxy_port, proxy_user, proxy_pass, user, passwd, FIRSTFROMNAME, LASTFROMNAME, start, end)
            SMTP_(index, name, proxy_host, proxy_port, proxy_user, proxy_pass, user, passwd, FIRSTFROMNAME, LASTFROMNAME, var.target.loc[start:end]).start()
    ...
    try:
        ...
        with open("config/report.csv", 'w', newline='', encoding="utf-8") as csvfile:
